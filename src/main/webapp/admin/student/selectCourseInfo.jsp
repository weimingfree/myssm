               ...
               <%-- <div id="top">
                ...
                <div id="middle">
                    <table class="table table-bordered table-striped">
                        ...
                        <c:forEach items="${info.list}" var="c"><%--这里原来是info.list，但视频中的代码是list--%>
                            <tr>
                                <td>${c.cid}</td>
                                <td>${c.cname}</td>
                                <td>${c.ctname}</td>
                                ...
                                <td>
                                    <button type="button" class="btn btn-warning" id="mydel"
                                            onclick="add('${sid}','${c.cid}','${c.cname}','${c.ctname}','${c.ctid}')">选课
                                    </button>
                                </td>
                            </tr>
                        </c:forEach>
...
<script type="text/javascript">
    function add(sid,cid,cname,ctname,ctid) {
         var str ="?sid="+sid+"&cid="+cid+"&cname="+cname+"&ctname="+ctname+"&ctid="+ctid;
        location.href = "${pageContext.request.contextPath}/selectCourse/add.action" + str ;
    }
    ...
    function ajaxsplit(page) {
        ...
        $.ajax({
            url:"${pageContext.request.contextPath}/selectCourse/ajaxSplit.action",
            data:{"spage":page},
            type:"post",
            success:function () {
                //重新加载显示分页数据的容器
                //location.href---->http://localhost:8080/admin/login.action
                $("#table").load("http://localhost:8080/admin/student/selectCourseInfo.jsp #table");
            }
        });
    }
    //查询
    function conditionByCname(){
        ...
        $.ajax({
            type:"post",
            url:"${pageContext.request.contextPath}/courseInfo/conditionByCname.action",
            data:{"cname":cname},
            success:function (){
                //刷新显示数据的容器
                $("#table").load("http://localhost:8080/admin/student/selectCourseInfo.jsp #table")
    ...
    function conditionByTname(){
        ...
        $.ajax({
            type:"post",
            url:"${pageContext.request.contextPath}/courseInfo/conditionByTname.action",
            data:{"tname":tname},
            success:function (){
                //刷新显示数据的容器
                $("#table").load("http://localhost:8080/admin/student/selectCourseInfo.jsp #table")
