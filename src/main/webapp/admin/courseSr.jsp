...
<div id="brall">
    ...
    <div id="condition" style="text-align: center">
        <form id="myform">
            学生姓名：<input name="selectsname" id="selectsname">
            <input type="button" value="查询" onclick="conditionBySname()">
        </form>
        <form id="myform2">
    ...
    <div id="table">
        <c:choose>
                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-info "
                                            onclick="one(${csr.hideid})">编辑
                                    </button>
                                    <button type="button" class="btn btn-warning" id="mydel"
                                            onclick="del(${csr.hideid})">删除
                                    </button>
                                </td>
                            </tr>
                        </c:forEach>
            ...
            <c:otherwise>
                <div>
                    <h2 style="width:1200px; text-align: center;color: orangered;margin-top: 100px">暂时没有符合条件的选课记录！</h2>
                </div>
            </c:otherwise>
        </c:choose>
    </div>
        ...
        if (confirm("确定删除吗")) {
            ...
            $.ajax({
                ...
                success:function (msg){
                    alert(msg);
                    $("#table").load("http://localhost:8080/admin/courseSr.jsp #table");
                }
            });
        }
    }
...
<script type="text/javascript">
    ...
    function conditionBySname(){
        //取出查询的条件
        var sname = $("#selectsname").val();
        $.ajax({
            type:"post",
            url:"${pageContext.request.contextPath}/courseSr/conditionBySname.action",
            data:{"sname":sname},
            success:function (){
                //刷新显示数据的容器
                $("#table").load("http://localhost:8080/admin/courseSr.jsp #table")
